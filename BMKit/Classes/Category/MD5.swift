...
//  ZLZZKit
//
//  Created by bymost on 2017/10/20.
//
...
extension HashProtocol {
    func prepare(_ len: Int) -> Array<UInt8> {
        ...
        tmpMessage.append(0x80) // append one bit (UInt8 with one bit) to messages
        ...
        tmpMessage += Array<UInt8>(repeatElement(0, count: counter))
        return tmpMessage
    }
}
...
public final class MD5: HashProtocol {
    ...
    func calculate() -> [UInt8] {
        ...
        // 2. Append length a 64-bits representation of lengthInBits
        let lengthInBits = (message.count * 8)
        let lengthBytes = lengthInBits.bytes(64 / 8)
        tmpMessage += lengthBytes.reversed()
        
        // Process the message in successive 512-bit chunks:
        let chunkSizeByte = 512 / 8
        
        for chunk in BytesSequence(chunkSize: chunkSizeByte, data: tmpMessage){
            // break chunk into sixteen 32-bit words m[j], 0 << j << 15
            var M = toUInt32Array(chunk)
            assert(M.count == 16, "Invalid Array")
            
            // Initialize hash value for ths chunk:
            var A: UInt32 = hv[0]
            var B: UInt32 = hv[1]
            var C: UInt32 = hv[2]
            var D: UInt32 = hv[3]
        ...
        var result = [UInt8]()
        result.reserveCapacity(hv.count / 4)
